...
function wppv_enqueue_scripts() {
	if( get_option('wppv_onoff') == 'yes' && (get_option('wppv_top') || get_option('wppv_bottom')) ) {
		...
		wp_register_script('wppvjs', WP_PLUGIN_URL .'/wp-postvoting/js/wp-postvoting.js', array('jquery'), '1.0');
		...
		wp_enqueue_style('wppvcss', WP_PLUGIN_URL .'/wp-postvoting/css/wp-postvoting.css?v=1.0');
	}
}
...
function wppv_get_highest_voted_posts($numberofpost) {
	...
	while ( $the_query->have_posts() ) : $the_query->the_post(); ?>
		<div class="wp_title">
			<h3><a href="<?php the_permalink() ?>" title="<?php get_the_title(); ?>" rel="bookmark"><?php echo wp_trim_words( get_the_title(), 12, ' ...' ).' ('.get_post_meta(get_the_ID(), '_wppvcount', true).')'; ?></a></h3>
			<div class="clear"></div>
		</div>
<?php
	endwhile;
...
function getPagingLink() {
	...
	if ( $pagingLink ) {
		echo '<div class="tablenav"><div class="tablenav-pages" style="margin: 1em 0">' . $pagingLink . '</div></div>';
	}
}
?>